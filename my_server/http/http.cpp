...
{
    return parseInternal(buf.c_str(), buf.size());
}

// 解析请求行
int HttpRequest::parseInternal(const char *buf, int size)
{
    ...
    while (_decodeState != HttpRequestDecodeState::INVALID && _decodeState != HttpRequestDecodeState::INVALID_METHOD && _decodeState != HttpRequestDecodeState::INVALID_URI && _decodeState != HttpRequestDecodeState::INVALID_VERSION && _decodeState != HttpRequestDecodeState::INVALID_HEADER && _decodeState != HttpRequestDecodeState::COMPLETE && _nextPos < size)
    {
        ...
        case HttpRequestDecodeState::URI_PARAM_VALUE:
        {
            if (ch == '&')
            {
                ...
                // 收获一个请求参数
                // 判断是否是HTTP报文长度字段
                // if (strcmp(requestParamKey.begin, "context-length") == 0)
                // {
                //     size = std::stoi(requestParamValue.begin);
                // }
        ...
        case HttpRequestDecodeState::HEADER_VALUE:
        {
            if (ch == CR)
            {
                ...
                // judge if header value is content-length
                // if (strcmp(headerValue.begin, "content-length") == 0)
                // {
                //     size = std::stoi(headerValue.begin);
                // }
...
{
    int i;
    if ((i = recv(fd, temp, sizeof(temp), MSG_DONTWAIT)) > 0)
    {
        buf.append(temp, i);
    }
}

int HttpRequest::parseHttp()
{
}
