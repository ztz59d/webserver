...
public:
    ...
    Timer(int gap, int circle, int epollfd) : epoll_fd(epollfd), m_gap(gap), m_circle(circle), m_timerQueue(m_circle, TimerList()), m_current(m_timerQueue.begin())
    {
    ...
    void expired()
    {

        {
            ...
            // m_current->erase(m_current->begin(), m_current->end());
            // m_current->clear();
        }
        // std::cout << "clear timer queue" << std::endl;
    }
